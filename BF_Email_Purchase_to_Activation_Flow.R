...
q1 = paste("
SELECT		so.order_date, so.order_number, so.email_address, so.fulfillment_status, so.financial_status, so.billing_full_name, ieh.Actual_Post_Goods_Issue_Date, ieh.Serial_Number, ieh.Material AS SKU, mp.Product_Family, mp.Product_Category, mp.Product_Name_Full
...
email_ship_data <- left_join(email_data, ship_data, by = c('order_number', 'email_address')) %>%
  ...
  select(order_date.x, category, order_number, email_address, fulfillment_status, financial_status, billing_full_name, quantity_sold, SKU.x, Actual_Post_Goods_Issue_Date, Serial_Number, Product_Family, Product_Category, Product_Name_Full) %>%
  `colnames<-` (c('order_date', 'category', 'order_number', 'order_email', 'fulfillment_status', 'financial_status', 'billing_full_name', 'quantity_sold', 'sku', 'shipment_date', 'mac_id', 'product_family', 'product_category', 'product_name_full'))


#Query activation data
q2 = paste("SELECT      dd.id, dd.doorbot_web_doorbot_id, dd.activation_time::TIMESTAMP AS activation_time, dhi.device_type, dhi.device_category, dhi.marketing_name, COALESCE(UPPER(md.mac_id), UPPER(dd.mac_address)) AS mac_id, UPPER(md.serial_number) AS serial_number, hdo.household_id, dwu.email AS activation_email, dwu.first_name + ' ' + dwu.last_name AS activation_full_name, hdo.device_order, hdo.ownership_time::TIMESTAMP AS ownership_time, dds.location_id, dc.iso_code, dc.country AS activation_country
...
write.xlsx(final_df %>% select(order_number, mac_id, sku, product_family, product_category, product_name_full, order_date, quantity_sold, order_email, billing_full_name, shipment_date, activation_email, activation_full_name, activation_time, ownership_time, device_order, activation_country), save_loc_output, row.names = FALSE)
